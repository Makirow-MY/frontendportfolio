...
export default function ShopSlug() {
  ...
  const currencyRates = {
    ...
    CL: { code: "CLP", symbol: "CLP$", rate: 937.5 },
    CM: { code: "XAF", symbol: "FCFA", rate: 608.25 },
    CN: { code: "CNY", symbol: "¥", rate: 7.24 },
    CO: { code: "COP", symbol: "COL$", rate: 3912.5 },
    ...
    GB_complex: { code: "GBP", symbol: "£", rate: 0.79 },
    ...
    ID: { code: "IDR", symbol: "Rp", rate: 15887.5 },
    ...
    KE: { code: "KES", symbol: "KSh", rate: 157.5 },
    ...
    NG: { code: "NGN", symbol: "₦", rate: 1505.5 },
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      const savedTransaction = window.localStorage.getItem("transaction");
      ...
      if (!savedTransaction) {
        ...
        window.localStorage.setItem(
          "transaction",
          JSON.stringify({ id: newTransactionId, status: "pending" })
        );
      } else {
  ...
  const calculateConvertedAmount = () => {
    const baseAmount = DataFromStorage.reduce((sum, item) => sum + item.price, 0);
  ...
  const validateStep = (step) => {
    ...
    if (step === 2) {
      ...
      if (formData.payment.method === "card") {
        ...
        else if (formData.payment.cardNumber.replace(/\s/g, "").length < 16) {
        ...
        else if (!/^(0[1-9]|1[0-2])\/?([0-9]{2})$/.test(formData.payment.expiry))
  ...
  const handleMoMoPayment = async () => {
    ...
    if (transactionStatus === "completed") {
      setErrors({ payment: "This transaction has already been completed." });
    ...
    try {
      const { amount, code } = calculateConvertedAmount();
      const response = await axios.post("/api/payment/momopayment", {
        action: "initiate",
        transactionId,
        amount,
        currency: code,
      ...
      if (response.data.status === "pending") {
        ...
        const interval = setInterval(async () => {
          attempts++;
          const statusResponse = await axios.post("/api/payment/momopayment", {
            action: "check-status",
            transactionId,
          });
          ...
          if (statusResponse.data.status === "SUCCESSFUL") {
            ...
            await storeTransaction("MTN MoMo");
            setTransactionStatus("completed");
            window.localStorage.setItem(
              "transaction",
              JSON.stringify({ id: transactionId, status: "completed" })
            );
            setPaymentSuccess(true);
            setIsProcessing(false);
          } else if (statusResponse.data.status === "FAILED" || attempts >= maxAttempts) {
            clearInterval(interval);
            setErrors({ payment: "MoMo payment failed or timed out." });
            setTimeout(() => setErrors({}), 3000);
            setIsProcessing(false);
          }
        }, 3000);
      } else {
        setErrors({ payment: "Failed to initiate MoMo payment." });
        setTimeout(() => setErrors({}), 3000);
        setIsProcessing(false);
      }
    } catch (error) {
      setErrors({ payment: "An error occurred during MoMo payment." });
      setTimeout(() => setErrors({}), 3000);
      setIsProcessing(false);
    }
  };

  const storeTransaction = async (paymentMethod) => {
    try {
      const { amount, code } = calculateConvertedAmount();
      await axios.post("/api/payment/storetransaction", {
        transactionId,
        formData,
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateStep(step)) {
      if (formData.payment.method === "MTN MoMo") {
        await handleMoMoPayment();
      } else if (formData.payment.method === "card") {
        setIsProcessing(true);
        setTimeout(async () => {
          await storeTransaction("Credit Card");
          setTransactionStatus("completed");
          window.localStorage.setItem(
            "transaction",
            JSON.stringify({ id: transactionId, status: "completed" })
  ...
  const paypalOptions = {
    ...
    currency: formData.personal.country ? (currencyRates[formData.personal.country]?.code || "USD") : "USD",
  };
    ...
    return (
      <div className="payment-success">
        ...
        <div className="receipt">
          <h3>Receipt</h3>
          <p>
            <strong>Amount:</strong> {convertedAmount.amount} {convertedAmount.code}
          </p>
          <p>
            <strong>Payment Method:</strong> {formData.payment.method}
          </p>
          <p>
        ...
        <button onClick={resetTransaction}>Make Another Payment</button>
      </div>
    );
  }
  ...
  return (
    <PayPalScriptProvider options={paypalOptions}>
      ...
      <section className="payment" onMouseMove={handleMouseMove} data-aos="fade">
        <div
        ...
        <div className="container">
          ...
          <div className="progress-steps" data-aos="fade-up" data-aos-delay="100">
            <div className={`step ${step >= 1 ? "active" : ""}`} data-aos="zoom-in" data-aos-delay="150">
              1. Personal Info
            </div>
            ...
            <div className={`step ${step >= 2 ? "active" : ""}`} data-aos="zoom-in" data-aos-delay="200">
              2. Payment
            </div>
            <div className="progress-bar">
              <div
            ...
            <div className={`step ${step >= 3 ? "active" : ""}`} data-aos="zoom-in" data-aos-delay="250">
              3. Billing
            </div>
          </div>

          <form className="leaveareplyform" style={{ width: "100%" }} onSubmit={handleSubmit}>
            {step === 1 && (
              <div className="form-step" style={{ width: "100%" }} data-aos="fade-up" data-aos-delay="200">
                <h2 data-aos="fade-right">Personal Information</h2>
                <div className="nameemailcomment" data-aos="fade-up" data-aos-delay="250">
                ...
                <div className="nameemailcomment" data-aos="fade-up" data-aos-delay="300">
                  <div className="form-group">
                    <label>Email*</label>
                    <input
                      type="email"
                ...
                <div className="form-row" data-aos="fade-up" data-aos-delay="400">
                  ...
                  <div className="form-group">
                    <label>Country*</label>
                    <select name="personal.country" value={formData.personal.country} onChange={handleCountryChange}>
                ...
                <div className="form-actions" data-aos="fade-up" data-aos-delay="450">
            ...
            {step === 2 && (
              <div className="form-step" data-aos="fade-up" data-aos-delay="200">
                <h2 data-aos="fade-right">Payment Method</h2>
                <div className="payment-methods" data-aos="fade-up" data-aos-delay="250">
                  <label
                  ...
                  <label
                    className={`payment-method ${formData.payment.method === "MTN MoMo" ? "selected" : ""}`}
                    data-aos="zoom-in"
                    data-aos-delay="400"
                  >
                    <input
                      ...
                      name="payment.method"
                      value="MTN MoMo"
                      checked={formData.payment.method === "MTN MoMo"}
                      onChange={handleChange}
                    />
                    <div className="method-content">
                      <div className="method-icon">🏦</div>
                ...
                {formData.payment.method === "card" && (
                  <div className="card-details" data-aos="fade-up" data-aos-delay="450">
                    <div className="nameemailcomment" data-aos="fade-up" data-aos-delay="500">
                      <div className="form-group">
                ...
                {formData.payment.method === "bank" && (
                  <div className="bank-details" data-aos="fade-up" data-aos-delay="450">
                    <div className="form-group">
                ...
                {formData.payment.method === "paypal" && (
                  <div className="paypal-notice" data-aos="fade-up" data-aos-delay="450">
                    ...
                    <PayPalButtons
                      createOrder={async () => {
                        try {
                          ...
                          setErrors({ payment: "Failed to initiate PayPal payment." });
                          setTimeout(() => setErrors({}), 3000);
                          return null;
                        }
                      }}
                      onApprove={async (data, actions) => {
                        ...
                        try {
                          ...
                          if (response.data.status === "COMPLETED") {
                            ...
                            setErrors({ payment: "PayPal payment failed." });
                            setTimeout(() => setErrors({}), 3000);
                          }
                        } catch (error) {
                          setErrors({ payment: "An error occurred during PayPal payment." });
                          setTimeout(() => setErrors({}), 3000);
                        } finally {
                          setIsProcessing(false);
                        }
                      ...
                      onError={() => {
                        setErrors({ payment: "PayPal payment was cancelled or failed." });
                        setTimeout(() => setErrors({}), 3000);
                      }}
                    />
                  </div>
                )}
                {formData.payment.method === "MTN MoMo" && (
                  <div className="paypal-notice" data-aos="fade-up" data-aos-delay="450">
                    <p>You will receive a prompt on your phone to approve the MTN MoMo payment.</p>
                  </div>
                )}
            ...
            {step === 3 && (
              <div className="form-step" data-aos="fade-up" data-aos-delay="200">
                ...
                <div className="summary-section" data-aos="fade-up" data-aos-delay="250">
                  <h3>Your Information</h3>
                  <div className="summary-grid">
                    <div>
                      <h4>Personal Details</h4>
                      <p>
                        <strong>Full Name:</strong> {formData.personal.firstName} {formData.personal.lastName}
                      </p>
                      <p>
                    ...
                    <div>
                      ...
                      <p>
                        <strong>City:</strong>
                        {formData.personal.city}, {formData.personal.zipCode}
                      </p>
                      <p>
                        <strong>Country:</strong>
                        {countryOptions
                          .filter((ab) => ab.value === formData.personal.country)
                          .map((ab) => ab.label)}
                      </p>
                    </div>
                    <div>
                      ...
                      {formData.payment.method === "card" && (
                        <p>
                          <strong>Card No: </strong> xxxx xxxx xxxx {formData.payment.cardNumber.slice(-4)}
                        </p>
                      )}
                      <p>
                        <strong>Currency: </strong>
                        {convertedAmount.symbol}, {convertedAmount.code}
                      </p>
                    </div>
                  </div>
                </div>
                <div className="summary-section" data-aos="fade-up" data-aos-delay="300">
                  <h3>Checkout Product{DataFromStorage.length > 1 ? "s" : null}</h3>
                  <div className="summary-grid">
                    {DataFromStorage.map((items, index) => (
                      <div className="summcard" key={index} data-aos="zoom-in" data-aos-delay={350 + index * 50}>
                ...
                <div className="order-summary" data-aos="fade-up" data-aos-delay="400">
                  <h3>Order Summary</h3>
                  ...
                  <div className="currency-note">
                    <p>
                      Amount converted to the currency of your country (
                      {countryOptions
                        .filter((ab) => ab.value === formData.personal.country)
                        .map((ab) => ab.label)}
                      , {convertedAmount.code}) using current exchange rates.
                    </p>
                  </div>
                   <div className="summary-item">
